...
def main():
    ...
    for root, directory_names, filenames in os.walk(top_level_directory):
        ...
        for filenm in filenames:
            # print("Trying to match " + logfile_pattern + "to a logfile " + filenm)

            if re.search( logfile_pattern , filenm) :
                print("Found a logfile " + filenm + " in " + root)
                ...
                metadata.at[rownum, 'DepositDescription'] = "Data generated by team member: " + person_name + " in folder: " + root  + ". Metadata compiled automatically by prepare_deposits_for_vault.py. "
                metadata.at[rownum, 'PathToFiles'] = root 
                logfile_relative_path = root + "\\" + filenm
                logfile = open(logfile_relative_path, "r")
                for each_line in logfile : 
                    if re.search(log_experiment, each_line):
                        print("Found expt info " + each_line)
                        metadata.at[rownum, 'DepositDescription'] += each_line 
                logfile.close()
                rownum += 1 
                continue
            #else: 
             #   match_date = re.search(date_folder_pattern, current_folder)
              #  if match_date: 
               #     metadata.at[rownum, 'DepositTitle'] = deposit_title_prefix + person_name + " (" + year_name + ") " + current_folder
                #    metadata.at[rownum, 'DepositDescription'] = each_line + " Data generated by team member: " + person_name + " in folder: " + current_folder  + ". Deposit contains files that were stored in " + root + ". Metadata compiled automatically by prepare_deposits_for_vault.py."
                 #   metadata.at[rownum, 'PathToFiles'] = root 
                  #  rownum += 1 
